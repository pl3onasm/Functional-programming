This implementation of Sudoku in Haskell is discussed in the 
first four video lectures of "Advanced Functional Programming" 
by Graham Hutton (a link to the YouTube playlist can be found 
...
-------------------------------------------------------------------
Also note that the function collapse is used in a different way
in solve4. In the earlier versions of the solver, collapse was used
to generate all possible grids, and then filter out the valid ones.
In solve4, however, collapse is used to unwrap the only grid that 
remains. After the pruning and expanding do all the heavy lifting, 
collapse is no longer part of the search process, but is just the  
final step to convert the matrix of choices into a grid (hence the
name "collapse", whereas the name "explode" would have been more
appropriate in the earlier versions of the solver).
---------------- Note added by David De Potter --------------------

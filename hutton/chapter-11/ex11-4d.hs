...
-- Finds a node in the game tree that matches the given grid
-- If the grid is not found, it raises an error
...
-- player O, which is used for evaluating moves
-- This tree is cached to avoid recomputing it every time
-- during the game
-- The tree is annotated with the best score for each grid
-- using alpha-beta pruning. It is important that we keep
-- the entire tree intact, otherwise findNode may fail
-- to find the correct node.
cachedTree :: Tree (Grid, Player)
cachedTree = annotateTree (gametree empty O)

-- Scores the game tree using alpha-beta pruning
abEval :: Player -> Grid -> Player
abEval p g = ab p O X (gametree g p)

-- Helper: evaluates the game tree using alpha-beta pruning
-- updating the player's score based on the game state
ab :: Player -> Player -> Player -> Tree Grid -> Player
ab _ _ _ (Node g [])
  | wins O g = O
  | wins X g = X
  | otherwise = B
ab p alpha beta (Node _ ts)
  | p == O = pruneMin ts beta
  | p == X = pruneMax ts alpha
  where
    pruneMin [] b = b
    pruneMin (t : ts) b =
      let v = ab (nextPlayer p) alpha b t
          b' = min b v
      in if alpha >= b' then b' else pruneMin ts b'

    pruneMax [] a = a
    pruneMax (t : ts) a =
      let v = ab (nextPlayer p) a beta t
          a' = max a v
      in if a' >= beta then a' else pruneMax ts a'

-- Annotates the full tree with alpha-beta-evaluated scores
annotateTree :: Tree Grid -> Tree (Grid, Player)
annotateTree (Node g ts) =
  Node (g, abEval (turn g) g) (map annotateTree ts)
...
{-
  This version uses a precomputed game tree annotated
  with the best scores for each grid state using the
  alpha-beta pruning technique. 
  The tree is generated once at the start of the game
  and reused for each turn, allowing the computer to
  make optimal decisions without recalculating the tree
  during the game.

  This works fine for a 3x3 grid, but for larger grids
  this approach fails due to the exponential growth
  of the game tree. That is why the computer player 
  hangs when the grid size is increased.

  In version 11.4.e, we will implement the alpha-beta
  pruning without precomputing the tree, allowing
  the computer to make optimal moves without needing
  to generate the entire game tree upfront.
